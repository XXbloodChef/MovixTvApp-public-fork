...
export const AdFreePopupProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const sync = () => {
      if (!isTvDevice() && getAdPopupMode() === 'normal' && SCRIPT_AD_MODE_ENABLED) loadAdScript();
    };
    sync();
  ...
  const showPopupForPlayer = useCallback((playerType: string, additionalInfo?: any) => {
    // Sur téléviseur, le parcours publicitaire est inapplicable : le popup exige
    // un clic sur un lien qui ouvre un onglet. Une télécommande ne sait pas le
    // faire et le shell WebView n'ouvrirait rien — le laisser actif bloquerait
