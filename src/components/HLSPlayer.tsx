...
import { Play, Pause, Volume2, VolumeX, Maximize, Minimize, Settings, Rewind, FastForward, Volume1, PictureInPicture, Users, Loader2, Repeat, Cast, Airplay, Info, X, Copy, Check, Lock, Scissors } from 'lucide-react';
import { isTvDevice, setNativeBackInterception } from '@/utils/tv/isTvDevice';
import { TvPlayerControls, type TvPlayerAction } from '@/tv/player/TvPlayerControls';
import { useTvPlayerRemote } from '@/tv/player/useTvPlayerRemote';
import { TvSeekFlash } from '@/tv/player/TvSeekFlash';
import { rememberWatchMeta } from '@/tv/data/useContinueWatching';
import { TvPlaybackNotice } from '@/tv/player/TvPlaybackNotice';
import { useTvPlaybackDiagnostic } from '@/tv/player/useTvPlaybackDiagnostic';
...
import HLSPlayerSettingsPanel, { HLSPlayerSourceMenu } from './HLSPlayerSettingsPanel';
import HLSPlayerSettingsPanelTv from './HLSPlayerSettingsPanelTv';
...
import { sniffCaptionLanguage } from '../utils/captionLanguageSniff';
import {
...
const dispatchDnsEmbedFallback = (
  ...
  // Une source qui injecte des publicités n'est jamais choisie sans l'accord
  // de l'utilisateur, même en dernier recours (voir sourceAdRisk.ts).
  if (omegaSources && omegaSources.length > 0 && !injectsAds('omega')) {
  ...
  if (coflixSources && coflixSources.length > 0 && !injectsAds('coflix')) {
...
/** Libellé d'une piste de sous-titres incrustés, dans la langue de l'interface. */
const embeddedCaptionsLabel = (channel: number): string => {
  const translated = i18next.t('watch.embeddedCaptions', { n: channel, defaultValue: '' });
  return translated || `Incrustés · CC${channel}`;
};
...
const createHlsConfig = (src: string) => {
  ...
  return {
    ...
    // Sous-titres incrustés (CEA-608) : hls.js les baptise « English », « Spanish »,
    // « Unknown CC » par convention américaine, sans rien savoir de la langue
    // réelle — sur Lanterns, « Unknown CC » était le français. Un nom neutre
    // avec le canal, sans code de langue, plutôt qu'une langue devinée.
    captionsTextTrack1Label: embeddedCaptionsLabel(1),
    captionsTextTrack1LanguageCode: '',
    captionsTextTrack2Label: embeddedCaptionsLabel(2),
    captionsTextTrack2LanguageCode: '',
    captionsTextTrack3Label: embeddedCaptionsLabel(3),
    captionsTextTrack3LanguageCode: '',
    captionsTextTrack4Label: embeddedCaptionsLabel(4),
...
/**
 * Remet les cues d'un même instant dans l'ordre de lecture, de haut en bas.
 *
 * Les sous-titres incrustés (CEA-608) arrivent de hls.js en un cue par rangée
 * d'écran, avec `line` = numéro de rangée. La liste native les rend dans un
 * ordre qui n'est pas celui des rangées — mesuré sur téléviseur le 12/09/2026
 * (Lanterns) : la rangée 15 avant la 14, donc la seconde ligne de la phrase
 * affichée au-dessus de la première. Les cues sans rangée (`line: 'auto'`,
 * sous-titres WebVTT ordinaires) gardent leur ordre relatif : le tri est stable
 * et ne compare que les rangées numériques.
 */
const sortCuesByRow = <T extends { line?: number | 'auto' | null }>(cues: T[]): T[] => {
  const row = (cue: T) => (typeof cue.line === 'number' ? cue.line : Number.POSITIVE_INFINITY);
  return cues
    .map((cue, index) => ({ cue, index }))
    .sort((a, b) => (row(a.cue) - row(b.cue)) || (a.index - b.index))
    .map(entry => entry.cue);
};
...
/** Setter inerte : sur téléviseur, la chrome souris ne s'ouvre jamais. */
const NOOP_SET_CONTROLS: React.Dispatch<React.SetStateAction<boolean>> = () => {};

interface HLSPlayerProps {
  /**
   * Force le mode téléviseur. Laissé indéfini, il est déduit de la détection
   * TV : les six pages qui montent ce lecteur n'ont ainsi rien à changer.
   */
  tvMode?: boolean;
  src: string;
  controls?: boolean;
...
const HLSPlayer = forwardRef<HLSPlayerRef, HLSPlayerProps>(({
  ...
  const [showControlsRaw, setShowControlsRaw] = useState(true);
  // Sur téléviseur, la chrome souris est remplacée par une surcouche
  // télécommande. Plutôt que de conditionner les 33 blocs qui en dépendent, on
  // fige l'état : chacun rend déjà `opacity: 0` et `pointerEvents: 'none'`
  // quand il est faux, donc ils disparaissent et cessent de capter les touches.
  const isTvPlayback = tvMode ?? isTvDevice();
  const showControls = isTvPlayback ? false : showControlsRaw;
  const setShowControls = isTvPlayback ? NOOP_SET_CONTROLS : setShowControlsRaw;

  // Aiguillage du panneau de réglages : drill-down piloté à la télécommande
  // sur téléviseur, barre d'onglets à la souris ailleurs. Les deux reçoivent
  // le même sac de props (cf. le point de montage, `{showSettings && …}`) —
  // le corps de chaque onglet vient du même code, seule l'enveloppe change.
  // `isTvPlayback` ne bouge pas au cours d'une lecture, le composant non plus.
  const SettingsPanel = isTvPlayback ? HLSPlayerSettingsPanelTv : HLSPlayerSettingsPanel;

  // Pilotage télécommande. Déclaré ici plutôt que dans un composant enfant pour
  // pouvoir réutiliser directement togglePlay/skipTime du lecteur.
  // `togglePlay` et `skipTime` sont définis bien plus bas dans ce composant ;
  // ces refs évitent d'avoir à déplacer 4 000 lignes pour les rendre visibles ici.
  const togglePlayRef = useRef<(() => void) | null>(null);
  const skipTimeRef = useRef<((seconds: number) => void) | null>(null);
  const tvActionsRef = useRef<TvPlayerAction[]>([]);
  const tvRemote = useTvPlayerRemote({
    // Uniquement l'instance principale. WatchMovie/WatchTv en montent plusieurs
    // autres — cachées (`controls={false}`) et le menu de sources
    // (`onlyQualityMenu`) — dont les hooks se disputeraient les flèches sur
    // `window`, ce qui rendait la télécommande inerte menu ouvert.
    enabled: isTvPlayback && controls && !onlyQualityMenu,
    actionCount: tvActionsRef.current.length,
    onTogglePlay: () => togglePlayRef.current?.(),
    onSeek: seconds => skipTimeRef.current?.(seconds),
    onActivate: index => tvActionsRef.current[index]?.onSelect(),
    onExit: () => window.history.back(),
  });

  // Explique les pannes muettes propres au téléviseur : absence de Widevine,
  // piste audio Dolby Digital+ non décodable. On ne les contourne pas, on les
  // nomme — un message clair vaut mieux qu'un écran noir inexpliqué.
  const tvPlaybackIssue = useTvPlaybackDiagnostic({
    enabled: isTvPlayback && controls && !onlyQualityMenu,
    videoRef,
    hlsRef,
  });

  // Le lecteur vit hors de TvLayout : c'est donc lui qui doit réclamer le
  // Retour au shell natif, sinon celui-ci reculerait dans l'historique sans
  // laisser la surcouche se fermer d'abord.
  useEffect(() => {
    // Même restriction que le hook : les instances cachées et celle du menu de
    // sources montent et se démontent en cours de route. Sans ce filtre, leur
    // démontage remettait le drapeau à faux, et le shell reculait alors dans
    // l'historique — ce qui faisait quitter l'épisode au lieu de fermer le menu.
    if (!isTvPlayback || !controls || onlyQualityMenu) return;
    setNativeBackInterception(true);
    return () => setNativeBackInterception(false);
  }, [isTvPlayback, controls, onlyQualityMenu]);
  ...
  /**
   * Expirations de fragment consécutives sur la source courante.
   *
   * `fragLoadTimeOut` ne correspondait à aucune branche du gestionnaire
   * d'erreurs : il finissait en simple avertissement, et une source dont les
   * segments n'arrivent jamais n'était abandonnée que par le minuteur de 45 s.
   * Mesuré sur téléviseur : trois expirations de 10 s, puis 15 s d'attente
   * inutile avant la bascule — alors que l'échec était acquis dès la deuxième.
   */
  const fragTimeoutStreakRef = useRef(0);
  ...
  const [, setIsKisskhSubtitleSelected] = useState(false);
  ...
  // Secondes réellement cumulées par la rafale, distinctes du nombre d'appuis :
  // c'est ce que la pastille TV annonce.
  const [forwardSkipSeconds, setForwardSkipSeconds] = useState(0);
  const [rewindSkipSeconds, setRewindSkipSeconds] = useState(0);
  ...
  const {
    ...
    cancelPreview: cancelSubtitlePreview,
    resetAppearance: resetSubtitleAppearance,
  } = useSubtitlePreferences();

  // Décalage temporel des sous-titres. C'est une correction de
  // synchronisation propre au contenu, pas un style : il ne se persiste pas
  // et repart à zéro à chaque film ou épisode. Il vivait dans
  // `subtitlePreferences.delay` (localStorage), où un +3 s posé sur un
  // épisode mal calé s'appliquait au suivant.
  const [subtitleDelay, setSubtitleDelayState] = useState(0);
  const setSubtitleDelay = useCallback((next: number) => {
    setSubtitleDelayState(clampSubtitleDelay(next));
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    const applySubtitlePreview = (event: Event) => {
      ...
      // Sur téléviseur, passer d'auto à manuel change la taille effective
      // même à `fontSizePx` égal : on compare les bases, pas le champ.
      const previousFontSize = getSubtitleBaseFontSizePx(previous, isTvPlayback);
      const nextFontSize = getSubtitleBaseFontSizePx(next, isTvPlayback);
      const geometryChanged = nextFontSize !== previousFontSize
      ...
      if (nextFontSize !== previousFontSize) cue.style.fontSize = `${nextFontSize * scale}px`;
  ...
  useEffect(() => {
    ...
    const handleLoadStart = () => {
      ...
      fragTimeoutStreakRef.current = 0; // Série propre à chaque nouvelle source.
    ...
    const handlePlaying = () => {
      ...
      setIsBuffering(false); // Not buffering
      fragTimeoutStreakRef.current = 0; // La source livre : la série repart de zéro.
      clearBufferingTimeout();
      clearSourceTimeout(); // Annuler le timeout car la source fonctionne
    ...
    if (isMP4) {
      ...
      // Un fragment qui arrive remet la série d'expirations à zéro : c'est ce qui
      // distingue un creux passager d'une source morte après un saut.
      hls.on(Hls.Events.FRAG_LOADED, () => {
        fragTimeoutStreakRef.current = 0;
      });
      ...
      hls.on(Hls.Events.ERROR, (_event, data) => {
        ...
        if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
          ...
          if (data.details === Hls.ErrorDetails.FRAG_LOAD_TIMEOUT) {
            fragTimeoutStreakRef.current += 1;
            // Deux expirations *consécutives* : la source ne livre plus. Le
            // compteur est remis à zéro par FRAG_LOADED, donc une lecture établie
            // qui traverse un creux réseau (un fragment tardif, puis les suivants
            // arrivent) n'est jamais abandonnée — seule une série ininterrompue
            // l'est. Mesuré sur téléviseur : après un saut à la position
            // sauvegardée, quatre expirations d'affilée sans un seul fragment,
            // alors que le garde « jamais joué » de la version précédente laissait
            // passer parce que les premiers segments à 0 s avaient déjà chargé.
            if (fragTimeoutStreakRef.current >= 2) {
              console.warn(
                `⏱️ ${fragTimeoutStreakRef.current} expirations de fragment consécutives — source abandonnée`,
              );
              fragTimeoutStreakRef.current = 0;
              requestHlsFallback();
              return;
            }
            console.log('🔄 Fragment load timeout - attempting recovery...');
            hls.startLoad();
            return;
          }
        }
    ...
    if (subtitleId.startsWith('internal:')) {
      ...
      if (track) {
        ...
        const reapplySelectedTrack = () => {
          ...
          if (selectedTrack) refreshActiveCues(video, selectedTrack, subtitleDelay);
        };
  ...
  const refreshActiveCues = (video: HTMLVideoElement, track: TextTrack, delay: number) => {
    ...
    const orderedCues = sortCuesByRow(dedupedCues);
    setActiveSubtitleCues(orderedCues);
    setSubtitleContainerVisible(orderedCues.length > 0);
  ...
  }, [activeTranslatedSubtitleId, isCasting, subtitleDelay, translatedSubtitleHistory]);
  ...
  const startSubtitleTranslation = useCallback(async () => {
    ...
    const sourceCues: SubtitleCueSnapshot[] = sortCuesByRow(Array.from(activeTrack.cues) as VTTCue[]).map(cue => ({
  ...
  }, [batchTranslateChunk, currentSubtitle, subtitleDelay, t, translateSubsTo]);
  ...
  const skipTime = (seconds: number) => {
    if (videoRef.current) {
      ...
      if (seconds > 0) {
        ...
        setForwardSkipSeconds(prev => prev + seconds);
        ...
        setRewindSkipSeconds(prev => prev + Math.abs(seconds));
  ...
  // Renseignés à chaque rendu : la surcouche TV appelle exactement les mêmes
  // fonctions que la barre souris, il n'y a pas de logique de lecture dupliquée.
  togglePlayRef.current = togglePlay;
  skipTimeRef.current = skipTime;
  // Trois boutons seulement. Lecture/Pause a sa touche dédiée sur la
  // télécommande, et OK s'en charge tant qu'aucun bouton n'est sélectionné :
  // lui garder une place obligeait à traverser un bouton inutile pour atteindre
  // les autres. Les sous-titres, eux, restent à un onglet du panneau.
  tvActionsRef.current = isTvPlayback
    ? [
        {
          id: 'rewind',
          label: 'Reculer de 10 secondes',
          icon: 'rewind',
          onSelect: () => skipTime(-10),
        },
        {
          id: 'forward',
        ...
        },
        {
          ...
          label: 'Réglages',
          icon: 'settings',
          // Ouvre sur la qualité : c'est le réglage qu'on vient changer dans
          // l'immense majorité des cas, et le reste du panneau reste accessible
          // par ses onglets une fois ouvert.
          onSelect: () => {
            setSettingsTab('quality');
            setShowSettings(true);
          },
  ...
  const createPipSubtitleTrack = useCallback(() => {
    ...
    const cues: SubtitleCueSnapshot[] = translated?.cues ?? sortCuesByRow(Array.from(activeTrack?.cues ?? []) as VTTCue[]).map(cue => ({
      startTime: cue.startTime,
      endTime: cue.endTime,
      text: (cue as VTTCue).text || '',
    }));
    ...
    const blobUrl = URL.createObjectURL(new Blob([
      cuesToWebVtt(cues, subtitleDelay),
  ...
  const buildCurrentCastTracks = useCallback(async () => {
    ...
    for (const entry of translatedSubtitleHistory) {
      candidates.push({
        ...
        vtt: cuesToWebVtt(entry.cues, subtitleDelay),
      });
    }
    ...
    if (video) {
      ...
      for (const [index, textTrack] of Array.from(video.textTracks).entries()) {
        ...
        const cues = sortCuesByRow(Array.from(textTrack.cues) as VTTCue[]).map(cue => ({
          startTime: cue.startTime,
          endTime: cue.endTime,
          text: (cue as VTTCue).text || '',
        }));
        candidates.push({
          ...
          vtt: cuesToWebVtt(cues, subtitleDelay),
        });
      }
    }
    ...
    activeTranslatedSubtitleId,
    currentSubtitle,
    preparedKisskhSubtitles,    subtitleDelay,
    subtitleUrl,
    t,
    translatedSubtitleHistory,
  ]);
  ...
  /**
   * Métadonnées d'affichage de la reprise de lecture.
   *
   * La rangée « Reprendre la lecture » de l'accueil TV lit les clés
   * `progress_*` écrites plus bas, qui ne contiennent que des nombres. Le
   * titre, l'image et le nom d'épisode n'y sont pas — mais le lecteur les a
   * sous la main à cet instant précis. Les déposer ici évite à l'accueil de
   * résoudre une douzaine d'identifiants contre TMDB à son montage, en
   * concurrence directe avec le chargement de ses propres rangées.
   *
   * On enregistre le chemin TMDB brut et non une URL construite : la taille
   * demandée dépend du gabarit de carte, qui n'est pas figé.
   *
   * Rien n'est purgé, volontairement. Les clés `progress_*` ne le sont pas non
   * plus ; évincer une métadonnée dont la progression survit ferait disparaître
   * de la rangée un titre pourtant reprenable.
   */
  useEffect(() => {
    const key = getProgressKey();
    const label = title || tvShow?.name;
    if (!key || !label) return;
    rememberWatchMeta(key, {
      title: label,
      backdropPath: currentEpisodeInfo?.still_path ?? tvShow?.backdrop_path ?? null,
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      switch (e.code) {
        case 'ArrowLeft':
          if (isTvPlayback) break;
          e.preventDefault();
          skipTime(-10);
          break;
        case 'ArrowRight':
          // Sur TV, les quatre flèches appartiennent à la surcouche
          // télécommande. Ce gestionnaire est attaché à `document` et se
          // déclenche donc AVANT celui de la surcouche, posé sur `window` :
          // sans ce garde, ouvrir la barre puis se déplacer entre les boutons
          // avançait le film à chaque appui.
          if (isTvPlayback) break;
          e.preventDefault();
          skipTime(10);
          break;
        case 'ArrowUp':
          // Sur TV, haut/bas naviguent dans la surcouche ; le volume reste géré
          // par la télécommande du téléviseur lui-même.
          if (isTvPlayback) break;
          e.preventDefault();
          {
  ...
  const handleHlsError = async () => {
    ...
    // 4. Si aucune source HLS n'a fonctionné, essayer les sources embed — sauf
    // celles qui injectent des publicités : elles restent au choix explicite de
    // l'utilisateur (voir sourceAdRisk.ts). Sans repli acceptable, l'étape 5
    // rend la main à la page, qui ouvre le menu des sources.
    if (!switched && omegaSources && omegaSources.length > 0 && !injectsAds('omega')) {
    ...
    if (!switched && coflixSources && coflixSources.length > 0 && !injectsAds('coflix')) {
  ...
  useEffect(() => {
    ...
    const handleCueChange = (event: Event) => {
      ...
      if (track && (track.mode === 'showing' || track.mode === 'hidden')) {
        refreshActiveCues(video, track, subtitleDelay);
      }
    };
    ...
    const handleTimeUpdate = () => {
      ...
      if (activeTrack) {
        refreshActiveCues(video, activeTrack, subtitleDelay);
      }
    };
  ...
  }, [isCasting, preparedKisskhSubtitles, subtitleDelay]);
  ...
  }, [subtitles.length, subtitleDelay]);

  /**
   * Langue devinée des pistes de sous-titres incrustées, par libellé de piste
   * (« Incrustés · CC1 »…). Le flux ne la dit pas ; on la lit dans le texte des
   * cues (cf. captionLanguageSniff). Remise à zéro à chaque contenu.
   */
  const [embeddedLanguageHints, setEmbeddedLanguageHints] = useState<Record<string, string>>({});
  useEffect(() => {
    setEmbeddedLanguageHints({});
  }, [contentQualityKey]);

  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    // Candidates : pistes de sous-titres incrustés (kind « captions »), sans
    // langue déclarée, pas encore identifiées. Sans candidate, pas de minuteur.
    const candidates = () => Array.from(video.textTracks).filter(track => (
      track.kind === 'captions' && !track.language && !(track.label in embeddedLanguageHints)
    ));
    if (candidates().length === 0) return;

    // Toutes les 5 s, au plus 60 fois (les cues s'accumulent au fil des
    // segments) : quelques dizaines de chaînes par passage, puis arrêt dès que
    // chaque piste est identifiée ou que le plafond est atteint.
    let attempts = 0;
    const timer = window.setInterval(() => {
      ...
      for (const track of pending) {
        // `cues` vaut null tant que la piste est « disabled » — or les pistes
        // non choisies le sont. On la passe en « hidden » le temps de lire,
        // puis on la remet, dans le même tour : l'événement `change` est
        // différé, le rendu et hls.js ne voient qu'une piste « disabled ».
        const wasDisabled = track.mode === 'disabled';
        if (wasDisabled) track.mode = 'hidden';
        const cues = track.cues ? Array.from(track.cues).slice(-120) : [];
        if (wasDisabled) track.mode = 'disabled';
        const result = sniffCaptionLanguage(cues.map(cue => (cue as VTTCue).text || ''));
        if (result.language) {
          const language = result.language;
          setEmbeddedLanguageHints(previous => (
            previous[track.label] === language ? previous : { ...previous, [track.label]: language }
          ));
        }
      }
    }, 5000);
    return () => window.clearInterval(timer);
  }, [subtitles, embeddedLanguageHints]);

  useEffect(() => {
    // Tenir les modes des pistes texte alignés sur la sélection.
    //
    // Le rendu maison affiche les cues de la première piste en mode « hidden »
    // ou « showing », sans vérifier que c'est la piste choisie. hls.js crée en
    // cours de lecture des pistes de sous-titres incrustés (CEA-608 : « English »,
    // « Unknown CC ») et les met d'office en « hidden » : mesuré sur téléviseur
    // le 12/09/2026 (Lanterns), des sous-titres anglais s'affichaient avec le
    // réglage sur « Désactivé », et ces pistes n'apparaissaient pas dans la liste
    // puisqu'elles arrivent après l'annonce des sous-titres du manifeste.
    //
    // À chaque ajout ou changement de piste, on réapplique donc la sélection
    // (`internal:N` → cette piste en « hidden », les autres « disabled » ; « off »
    // ou piste traduite → toutes « disabled ») et on rafraîchit la liste du
    // panneau pour que les pistes incrustées soient visibles et activables.
    const syncTextTracks = () => {
      ...
      const tracks = Array.from(video.textTracks);
      const selection = currentSubtitleRef.current;
      const managed = selection === 'off'
        || /^internal:\d+$/.test(selection)
        || selection.startsWith('translated:');

      if (managed) {
        applySelectedTextTrackMode(tracks, selection);
        if (selection === 'off') {
          setActiveSubtitleCues([]);
          setSubtitleContainerVisible(false);
        }
      ...
      // Sélection hors de ce schéma : on se contente d'empêcher le rendu natif.
    ...
    const handleTrackAdded = () => {
      const video = videoRef.current;
      if (video) setSubtitles(Array.from(video.textTracks));
      syncTextTracks();
    };
  ...
  // ── Sac de props du panneau de réglages ─────────────────────────────────────
  // Les deux panneaux sont `React.memo`, mais un objet littéral recréé à chaque
  // rendu — et ce lecteur se rend à chaque `timeupdate` — leur donnait de
  // nouvelles props à chaque fois : mesuré sur téléviseur, la coquille TV se
  // rendait ~20 fois par seconde, panneau ouvert. Huit fonctions du lecteur
  // changeaient elles-mêmes d'identité à chaque rendu ; enveloppées ci-dessous,
  // elles gardent la même référence et appellent toujours la version courante.
  // Le sac n'est alors recomposé que lorsqu'une valeur qu'il contient change.
  // Placé avant le retour anticipé `onlyQualityMenu` pour respecter l'ordre des
  // hooks ; `zoomState` et `resetZoom`, déclarés plus bas, sont passés à part.
  const stableHandleSourceChange = useStableCallback(handleSourceChange);
  const stableGetLanguageName = useStableCallback(getLanguageName);
  const stableHandleAudioTrackChange = useStableCallback(handleAudioTrackChange);
  const stableHandleSubtitleChange = useStableCallback(handleSubtitleChange);
  const stableLoadExternalSubtitle = useStableCallback(loadExternalSubtitle);
  const stableRefreshActiveCues = useStableCallback(refreshActiveCues);
  const stableHandlePlaybackSpeedChange = useStableCallback(handlePlaybackSpeedChange);
  const stableResetCurrentProgress = useStableCallback(resetCurrentProgress);
  ...
  const settingsPanelProps = useMemo(() => ({
    ...
    onlyQualityMenu,
    embedType,
    handleSourceChange: stableHandleSourceChange,
    getSourceQualityLabel,
    ...
    getLanguageName: stableGetLanguageName,
    currentAudioTrack,
    handleAudioTrackChange: stableHandleAudioTrackChange,
    currentSubtitle,
    handleSubtitleChange: stableHandleSubtitleChange,
    episodeNumber,
    seasonNumber,
    movieId,
    externalTracks,
    ...
    loadingSubtitle,
    loadExternalSubtitle: stableLoadExternalSubtitle,
    setLoadingSubtitle,
    selectedExternalSub,
    setCurrentSubtitle,
    setSubtitleContainerVisible,
    refreshActiveCues: stableRefreshActiveCues,
    subtitlePreferences,
    ...
    setSubtitleDelay,
    playbackSpeed,
    handlePlaybackSpeedChange: stableHandlePlaybackSpeedChange,
    saveProgressEnabled,
    ...
    resetCurrentProgress: stableResetCurrentProgress,
    audioEnhancerMode,
    ...
    handleSkipSettingsChange,
    handleSkipSettingsReset,
    handleSourceMenuFocusCapture,
    handleVideoOledChange,
    handleVolumeBoostChange,
    hlsQualityScan,
    ...
    sourceMenuRef,
    src,
    stableGetLanguageName,
    stableHandleAudioTrackChange,
    stableHandlePlaybackSpeedChange,
    stableHandleSourceChange,
    stableHandleSubtitleChange,
    stableLoadExternalSubtitle,
    stableRefreshActiveCues,
    stableResetCurrentProgress,
    startSubtitleTranslation,
    subtitleDelay,
    setSubtitleDelay,
    subtitlePreferences,
  ...
  // Mode « liste des sources seule » (`onlyQualityMenu`) : WatchMovie/WatchTv
  // montent un HLSPlayer sans vidéo à côté d'un lecteur embed pour proposer le
  // changement de source. Le corps est celui de l'onglet Sources du panneau
  // souris (`HLSPlayerSourceMenu`), plus une copie inline. Placé après tous les
  // hooks du composant : un `return` avant eux en faisait des hooks
  // conditionnels (dix erreurs `rules-of-hooks`, et un ordre d'appel qui aurait
  // changé si `onlyQualityMenu` variait en cours de vie).
  if (onlyQualityMenu) {
    return (
      <div
        ref={sourceMenuRef}
      ...
      >
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="pr-2"
          >
            {settingsTab === 'quality' && <HLSPlayerSourceMenu {...settingsPanelProps} />}
          </motion.div>
        </AnimatePresence>
      </div>
    );
  ...
  return (
    <div
      ...
      // Plein écran permanent sur téléviseur. Style inline et non classes :
      // `relative` et `fixed` sont deux utilitaires Tailwind de même
      // spécificité, l'ordre dans l'attribut ne déciderait pas du gagnant.
      // z-index volontairement bas — l'overlay du menu de sources est en 10000.
      style={
        isTvPlayback && controls && !onlyQualityMenu
    ...
    >
      ...
      {!isCasting && (
        ...
        >
          {isTvPlayback && controls && !onlyQualityMenu && (
            <TvPlaybackNotice issue={tvPlaybackIssue} />
          )}
          {isTvPlayback && controls && !onlyQualityMenu && (
            <TvPlayerControls
              visible={tvRemote.visible}
              // Même chaîne de repli que la chrome souris du lecteur : le prop
              // `title` n'est pas fourni par toutes les pages Watch.
              title={title || tvShow?.name || undefined}
              subtitle={
              ...
              qualityLabel={
                effectiveQualityHeight ? `${effectiveQualityHeight}p` : undefined
              }
              actions={tvActionsRef.current}
              focusedAction={tvRemote.focusedAction}
      ...
      <AnimatePresence>
        ...
        <SettingsPanel
          key="settings-panel"
          {...settingsPanelProps}
          // Déclarés après le retour anticipé `onlyQualityMenu`, donc hors du
          // sac mémoïsé ; `resetZoom` est déjà un `useCallback`, le memo tient.
          zoomState={zoomState}
          resetZoom={resetZoom}
        />
        )}
