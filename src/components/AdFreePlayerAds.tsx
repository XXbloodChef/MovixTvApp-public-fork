...
const AdFreePlayerAds: React.FC<AdFreePlayerAdsProps> = ({
  ...
  // `shouldShow` vrai quel que soit l'état du contexte. Le garde-fou doit donc
  // être ici aussi, pas seulement dans AdFreePopupContext.
  // Mémoïsé : la détection lit localStorage et analyse l'UA, inutile à chaque rendu.
  const isTv = React.useMemo(() => isTvDevice(), []);

  const finalOnAccept = onAccept || handlePopupAccept;
  const shouldShow = !isTv && (!!onClose || showAdFreePopup);
