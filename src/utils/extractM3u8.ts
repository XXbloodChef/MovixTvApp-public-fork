...
async function tryExtensionFirst(type: string, url: string, serverFallback: () => Promise<M3u8Result | null>): Promise<M3u8Result | null> {
  if (usesServerExtraction()) {
    const fromServer = await serverFallback();
    if (fromServer?.success) return fromServer;
    // Repli local (TV) : le serveur n'a rien résolu pour ce lien alors que le
    // bridge est là. Mesuré le 12/09/2026 sur six titres Coflix : uqload et
    // vidmoly ne sont jamais résolus côté serveur, voe une fois sur deux. Ces
    // liens restaient alors en iframe, avec les publicités de l'hébergeur.
    // Le bridge sait extraire ces mêmes hébergeurs en local ; on ne l'appelle
    // qu'APRÈS le registre serveur, jamais à sa place — le contrat « pas de
    // bascule de méthode » reste vrai pour l'inverse (local → serveur).
    if (!hasNexusExtractors() || !window.movixExtractM3u8) return fromServer;
    try {
      console.log(`[NEXUS] Serveur sans résultat pour ${type}, extraction locale: ${url}`);
      const local = await window.movixExtractM3u8(type, url);
      if (local && local.success) {
        console.log(`[NEXUS] Extraction locale réussie pour ${type} (repli après serveur)`);
        return local;
      }
      return fromServer ?? local;
    } catch (e) {
      console.warn(`[NEXUS] Repli local en échec pour ${type}:`, e);
      return fromServer;
    }
  }
