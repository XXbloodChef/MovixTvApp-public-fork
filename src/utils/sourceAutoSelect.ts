...
import { injectsAds } from './sourceAdRisk';
import type {
...
export function pickAutoSelectedSource(
  ...
  for (const { id, enabled } of sourceOrder) {
    if (!enabled) continue;
    // Jamais choisie d'office : une source qui injecte des publicités attend un
    // choix explicite (voir sourceAdRisk.ts). Le dernier lecteur mémorisé,
    // ci-dessus, en est un.
    if (injectsAds(id)) continue;
