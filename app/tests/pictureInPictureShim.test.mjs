...
async function loadInjectedJavaScriptBuilder() {
  ...
  const require = id => {
    ...
    if (id === './tv-shim') return { buildTvShim: () => 'TV_SHIM' };
...
test('injected runtime preserves PiP and playback ordering', async () => {
  ...
  const order = ['PIP_SHIM', 'PLAYBACK_AWAKE_SHIM', 'BRIDGE_RUNTIME', 'USERSCRIPT_SOURCE']
