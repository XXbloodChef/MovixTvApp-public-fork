...
import { getAdBlockStats, setAdBlockEnabled } from './adBlock';
import { DEV_SITE_URL } from '../config';
import {
  clearBackInterception,
  requestBackInterceptionResync,
  setBackInterception,
} from './tvBack';
import {
...
const MOVIX_TV_BACK_V1 = 'MOVIX_TV_BACK_V1';
...
export function clearBridgeCapabilities(
  ...
  clearBackInterception(webViewRef);
  // La page reste peut-être la même (iframe, redirection) : on lui demande de
  // redéclarer. Si elle a réellement changé, l'appel est sans effet.
  requestBackInterceptionResync(webViewRef);
}
...
export function isTrustedMovixBridgeUrl(
  ...
  // Serveur de développement. `DEV_SITE_URL` sert la page en `http://localhost`,
  // que la règle https ci-dessous rejette toujours : en debug, aucun message du
  // pont n'était de confiance — Retour reculait dans l'historique au lieu de
  // fermer le panneau, le maintien d'éveil et le proxy média étaient refusés
  // (mesuré sur téléviseur : `TV_BACK_INTERCEPT enabled=true trusted=false`).
  // L'exception ne vaut que pour un build debug ET l'origine exacte du serveur
  // de dev ; la release garde la règle stricte, mot pour mot.
  if (isDevSiteOrigin(source.origin)) return true;
...
function isDevSiteOrigin(origin: string): boolean {
  // `typeof` : les tests Node chargent ce module sans la globale de React Native.
  if (typeof __DEV__ === 'undefined' || !__DEV__ || !DEV_SITE_URL) return false;
  return parseAbsoluteHttpUrl(DEV_SITE_URL)?.origin === origin;
}
...
export async function handleBridgeMessage(
  ...
  if (parsed && typeof parsed === 'object') {
    ...
    if (p.type === 'TV_BACK_INTERCEPT') {
      // Même verrou que PLAYBACK_AWAKE_SET : origine de confiance et frame
      // principale. Une iframe hostile ne doit pas pouvoir capturer le Retour et
      // enfermer l'utilisateur dans la page.
      if (
        trustedTopFrame
        && p.capability === MOVIX_TV_BACK_V1
        && typeof p.enabled === 'boolean'
      ) {
        setBackInterception(webViewRef, p.enabled);
      }
      return;
    }
  ...
  switch (req.type) {
    ...
    case 'GM_JOURNAL_CONSOLE':
      ...
      break;
    // Bloqueur de publicités : état + bascule pour la page de réglages TV.
    case 'ADBLOCK_STATE':
      try {
        response = { id: req.id, success: true, value: await getAdBlockStats() };
      } catch (error) {
        response = { id: req.id, success: false, error: String(error) };
      }
      break;
    case 'ADBLOCK_SET':
      try {
        await setAdBlockEnabled(req.value === true);
        response = { id: req.id, success: true, value: await getAdBlockStats() };
      } catch (error) {
        response = { id: req.id, success: false, error: String(error) };
      }
